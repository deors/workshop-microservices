...
Workshop with detailed step by step instructions on how to build an exemplar microservices stack based in Spring Boot, Spring Cloud and Netflix OSS.

This workshop is organised in lab iterations, starting from the basics and building up in complexity and features along the way.
...
It is also possible to use the local repository, without pushing it to a remote repository. A local repository is enough to test services locally (Lab iteration 1), but a remote repository will be needed later, as services are distributed across multiple nodes and hence the local repository might not be accessible to every service instance.
...
## Lab iteration 2) Adding Docker images and running services in Docker Swarm
...
## Lab iteration 3) Configuring continuous integration pipelines with Jenkins
...
The detailed instructions to set up and configure Jenkins and SonarQube can be read in the [`workshop-pipelines` workshop repository](https://github.com/deors/workshop-pipelines).
